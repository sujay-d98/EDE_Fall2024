...
```{r}
#Exercise 1:
plot1 <- ggplot(EPAair, aes(x = Ozone, 
                            fill = as.factor(Years))) +
        geom_density(alpha = 0.5) +
  scale_fill_manual(calues = c("darkgreen", "darkblue"))+
  labs(x = "Ozone AQI Value",
       y="")
print(plot1)

plot2 <- ggplot(EPAair, aes(x=as.factor(Year),
                             y= Ozone))+
  geom_boxplot()+
...
violin <- ggplot(EPAair, aes(x=as.factor(Year),
                             ))
```
...
OzoneALLreg <- lm(data = EPAair, PM2.5 ~ mean_Lat + mean_Lng + Month + Year + Ozone )
step(OzoneALLreg)
...
OzoneBest.reg <- lm(data = EPAair, PM2.5 ~ mean_Lat + mean_Lng + Month + Ozone )
